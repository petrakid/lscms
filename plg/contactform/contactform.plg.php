<link rel="stylesheet" href="<?php echo $gbl['site_url'] ?>/css/form-render.css" />
...
if(isset($_SESSION['isLoggedIn'])) {
     $sqlf = $db->query("SELECT f_id, form_data FROM tbl_contactform WHERE page_id = $pg[p_id]");
     if($sqlf->rowCount() == 0) {
          $db->exec("INSERT INTO tbl_contactform (page_id) VALUES ('$pg[p_id]')");
          $sqlf = $db->query("SELECT f_id, form_data FROM tbl_contactform WHERE page_id = $pg[p_id]");          
     }
     $frmd = $sqlf->fetch(PDO::FETCH_ASSOC);
     ?>
     <link rel="stylesheet" href="<?php echo $gbl['site_url'] ?>/css/form-builder.css" />     
     ...
     <script>
     $(function() {
          var fbTemplate = $('#formBuilder'),
          options =  {
               dataType: 'xml',
               formData: `<?php echo $frmd['form_data'] ?>`
          };
          if(fbTemplate) {
