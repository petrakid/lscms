...
if(isset($_POST['unsubscribeall'])) {
     $sql = $db->query("SELECT subscriber_email FROM tbl_mailing_subscribers WHERE s_id = $_POST[subid]");
     $sub = $sql->fetch(PDO::FETCH_ASSOC);
     $lsts = $db->query("SELECT s_id FROM tbl_mailing_subscribers WHERE subscriber_email = '$sub[subscriber_email]'");
     while($lst = $lsts->fetch(PDO::FETCH_ASSOC)) {
          $db->exec("UPDATE tbl_mailing_subscribers SET subscriber_status = 0 WHERE s_id = $lst[s_id]");          
     }     
     ?>
     <div class="card-header"><h4 class="card-title">Success!</h4></div>
     <div class="card-body">You have been unsubscribed from ALL mailing lists.  You may now close this window.</div>     
     <?php
}
...
if(isset($_POST['unsubscribeselect'])) {
     ...
     echo '<div class="card-header"><h4 class="card-title">Success!</h4></div>
     <div class="card-body">You have been unsubscribed from the selected lists.</div>';
}

if(isset($_POST['subscribeme'])) {
     $ipaddress = $_SERVER['REMOTE_ADDR'];
     $fwdip = $_SERVER['HTTP_X_FORWARDED_FOR'];
     $metadata = $_SERVER['HTTP_USER_AGENT'] .'/'. $_SERVER['REMOTE_HOST'] .'/'. $_SERVER['HTTP_REFERER'];  
     $answer = 17;
