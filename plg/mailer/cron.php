<?php
if(!isset($_SESSION['sitevars'])) {
     die;
}

$sqlm = $db->query("SELECT * FROM $_SESSION[prefix]_mailings WHERE mailing_status = 3");
while($ml = $sqlm->fetch(PDO::FETCH_ASSOC)) {
     $lst = $db->query("SELECT s_id, subscriber_name, subscriber_email FROM $_SESSION[prefix]_mailing_subscribers WHERE subscriber_status = 1 AND subscriber_list_id = $ml[mailing_list_id]");
     $set = $db->query("SELECT * FROM $_SESSION[prefix]_mailing_settings WHERE mailer_list_id = $ml[mailing_list_id]");
     if(sendMailer($lst, $set, $ml)) {
          $db->exec("UPDATE $_SESSION[prefix]_mailings SET mailing_status = 1 WHERE m_id = $ml[m_id]");
     }
}
